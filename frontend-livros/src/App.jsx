...
function App() {
  ...
  // Efeito para calcular a paginação baseada na altura da tela
  useEffect(() => {
    const calcularPaginas = () => {
      if (textoRef.current) {
        // Altura da área de texto visível
        const alturaVisivel = textoRef.current.parentElement.clientHeight;
        // Altura total do texto
        const alturaTotal = textoRef.current.scrollHeight;

        const paginas = Math.ceil(alturaTotal / alturaVisivel);
        setTotalPaginas(paginas > 0 ? paginas : 1);

        if (paginaAtual >= paginas) setPaginaAtual(Math.max(0, paginas - 1));
      }
    };

    if (capituloLendo) {
      setTimeout(calcularPaginas, 200);
      window.addEventListener('resize', calcularPaginas);
      return () => window.removeEventListener('resize', calcularPaginas);
    }
  }, [capituloLendo, paginaAtual]);
  ...
  const fazerCadastro = async () => {
    ...
    } catch (erro) { setMensagem('Erro de conexão com o servidor.'); }
  }
  ...
  const salvarLivro = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(`Erro ao salvar: ${erroData.detail || "Verifique os campos."}`);
      }
    } catch (e) {
  ...
  // ==========================================
  // RENDERIZAÇÃO: TELA DE LOGIN (ATUALIZADA)
  // ==========================================
  if (mostrarLogin) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#f8f7fb] px-4 font-sans relative">
        ...
        <div className="bg-white p-8 rounded-3xl w-full max-w-md border border-gray-100 shadow-sm">
          ...
          {mensagem && (
            <div className={`p-3 rounded-xl mb-4 text-sm font-medium text-center ${mensagem.includes('sucesso') ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-600'}`}>
              {mensagem}
            </div>
          )}
          ...
          <form
            onSubmit={(e) => {
              ...
              if (!emailRegex.test(email)) {
                setMensagem("Por favor, insira um e-mail válido (ex: seuemail@dominio.com)");
                return;
              }

              modoLogin ? fazerLogin() : fazerCadastro();
            }}
          ...
          >
            ...
            >
              {modoLogin ? "Criar nova conta" : "Já tenho conta"}
            </button>
          </form>
        </div>
      </div>
  ...
  // ==========================================
  // RENDERIZAÇÃO: LEITURA E CAPÍTULOS (ALINHAMENTO E LAYOUT BLINDADO)
  // ==========================================
  if (livroSelecionado) {
    ...
    return (
      <div className="min-h-screen bg-white text-gray-900 p-4 sm:p-8 font-sans pb-24">
        ...
        {/* HEADER DA PÁGINA DO LIVRO (Resolve o problema de alinhamento das caixas) */}
        <div className="flex justify-between items-center w-full max-w-6xl mx-auto mb-4">
          <button
            onClick={() => { setLivroSelecionado(null); if (!usuarioExibido) setAbaAtual('inicio'); }}
            className="text-[#5a31f4] hover:underline font-semibold text-sm flex items-center gap-2"
          >
            ← Voltar
          </button>
        ...
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row gap-8 items-start">
          ...
          {/* COLUNA ESQUERDA: CAPA E INFO */}
          <div className="w-full md:w-[300px] flex-shrink-0 flex flex-col gap-4 sticky top-8">
            <img src={livroSelecionado.url_capa} className="w-full h-72 object-cover rounded-2xl shadow-md" alt="Capa" />

            <div>
              <h1 className="text-2xl font-bold leading-tight">{livroSelecionado.titulo}</h1>
              <p className="text-sm text-gray-500 font-medium mb-3">
                Por <span onClick={(e) => { e.stopPropagation(); irParaPerfilAutor(livroSelecionado.autor_id); }} className="text-[#5a31f4] hover:underline cursor-pointer font-bold">@{livroSelecionado.autor?.username?.replace('@', '')}</span>
              </p>
              <div className="flex gap-4 text-xs font-bold text-gray-400">
                <span>👁 {livroAtualizado.visualizacoes || 0} Visitas</span>
                <span>♡ {livroAtualizado.curtidas_totales || 0} Curtidas</span>
              </div>
            </div>

            <p className="text-gray-600 text-sm leading-relaxed max-h-64 flex-grow overflow-y-auto custom-scrollbar pr-2">
          ...
          {/* COLUNA DIREITA: LEITURA OU ÍNDICE */}
          <div className="flex-1 w-full min-w-0">
            {capituloLendo ? (
              <div className="bg-[#f8f7fb] p-6 sm:p-8 rounded-3xl border border-gray-100 h-[75vh] md:h-[80vh] flex flex-col shadow-sm">
                <div className="flex-shrink-0 mb-4">
                  <button onClick={() => { setCapituloLendo(null); setPaginaAtual(0); }} className="text-gray-500 font-bold text-sm hover:text-[#5a31f4] mb-2">← Voltar ao Índice</button>
                  <h2 className="text-xl font-bold mt-2 truncate">{capituloLendo.titulo_do_capitulo}</h2>
                ...
                <div className="flex-1 overflow-hidden relative min-h-0">
                  <div
                    ref={textoRef}
                    className="h-full pb-6 text-gray-800 font-serif text-lg leading-relaxed transition-transform duration-300 ease-in-out"
                    style={{
                      transform: `translateY(calc(-${paginaAtual} * 100%))`
                    }}
                  >
                ...
                {/* Controles */}
                <div className="flex-shrink-0 flex items-center justify-between pt-4 mt-4 border-t border-gray-200">
                  <button disabled={paginaAtual === 0} onClick={() => setPaginaAtual(p => p - 1)} className="px-4 sm:px-6 py-2.5 bg-white rounded-xl shadow-sm text-sm font-bold disabled:opacity-30">Anterior</button>
                  <span className="text-xs font-bold text-gray-400 tracking-widest">{paginaAtual + 1} / {totalPaginas}</span>
                  <button onClick={() => paginaAtual >= totalPaginas - 1 ? setCapituloLendo(null) : setPaginaAtual(p => p + 1)} className={`px-4 sm:px-6 py-2.5 text-white rounded-xl text-sm font-bold shadow-md ${paginaAtual >= totalPaginas - 1 ? 'bg-green-500 hover:bg-green-600' : 'bg-[#5a31f4] hover:bg-[#4924c9]'}`}>
                    {paginaAtual >= totalPaginas - 1 ? "Finalizar ✓" : "Próxima"}
                  </button>
                </div>
              </div>
            ) : (
              <div className="overflow-y-auto h-full pr-2 custom-scrollbar">
                {souDonoDoLivro && (
                  <div className="bg-white rounded-2xl p-6 mb-8 border border-gray-100 shadow-sm">
                    ...
                    <form onSubmit={salvarCapitulo} className="flex flex-col gap-4">
                      <input type="text" placeholder="Título do Capítulo" value={tituloCapitulo} onChange={(e) => setTituloCapitulo(e.target.value)} required className="p-4 rounded-xl bg-gray-50 border border-gray-100 focus:ring-2 focus:ring-[#5a31f4] outline-none w-full" />
                      <textarea placeholder="Escreva sua história aqui..." value={conteudoCapitulo} onChange={(e) => setConteudoCapitulo(e.target.value)} required className="p-4 rounded-xl bg-gray-50 border border-gray-100 focus:ring-2 focus:ring-[#5a31f4] outline-none w-full h-48 resize-y" />
                ...
                <div className="space-y-3">
                  {capitulos.length === 0 ? (
                    <p className="text-gray-500 py-8 bg-white text-center rounded-2xl border border-dashed border-gray-200">Nenhum capítulo publicado ainda.</p>
                  ) : (
                    capitulos.map((cap) => (
                      <div key={cap.id} className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 hover:border-[#5a31f4] transition-colors cursor-pointer group" onClick={() => iniciarLeituraCapitulo(cap)}>
                        <div>
                          <h4 className="font-bold text-gray-900 text-sm group-hover:text-[#5a31f4] transition-colors">Capítulo {cap.ordem_leitura}: {cap.titulo_do_capitulo}</h4>
                        ...
                        <div className="flex gap-2 w-full sm:w-auto" onClick={(e) => e.stopPropagation()}>
                          <button onClick={() => iniciarLeituraCapitulo(cap)} className="flex-1 sm:flex-none px-6 py-2 bg-[#f3efff] text-[#5a31f4] font-bold rounded-xl text-xs hover:bg-[#e9e2ff] transition-colors">Ler</button>
                          {souDonoDoLivro && (
                            <>
                              <button onClick={() => iniciarEdicaoCapitulo(cap)} className="px-4 py-2 bg-gray-100 text-gray-700 font-bold rounded-xl text-xs hover:bg-gray-200">Editar</button>
                              <button onClick={() => deletarCapitulo(cap.id)} className="px-4 py-2 bg-red-50 text-red-500 font-bold rounded-xl text-xs hover:bg-red-100">Excluir</button>
                            </>
                          )}
                        </div>
                      </div>
